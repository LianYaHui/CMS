...
<body>
    <div class="formbody" id="divFormContent" style="padding-bottom:0;">
        ...
        <label class="chb-label-nobg" style="float:left;"><input type="checkbox" class="chb" id="chbEnabled" /><span>启用遮挡报警</span></label>
            ...
            <div style="float:left;">
                ...
                <div style="clear:both;overflow:hidden;">
                    ...
                    <label class="chb-label-nobg" style="margin:0 10px 0 5px;">
                        <input type="radio" class="chb" name="rbAlarmSensitivity" value="1" checked="checked" /><span>低</span>
                    </label>
                    <label class="chb-label-nobg" style="margin-right:10px;">
                        <input type="radio" class="chb" name="rbAlarmSensitivity" value="2" /><span>中</span>
                    </label>
                    <label class="chb-label-nobg">
                        <input type="radio" class="chb" name="rbAlarmSensitivity" value="3" /><span>高</span>
                    </label>
                </div>
            </div>
            <div style="float:left;">
                ...
                <div id="divData" class="div_data" style="line-height:20px;margin:5px 0 0;"></div>
    ...
    <div class="formbottom">
        <a class="btn btnc24" id="btnRead" onclick="remoteConfig.readMaskAlarm(this);"><span>读取遮挡报警</span></a>
        <a class="btn btnc24" id="btnSet" onclick="remoteConfig.setMaskAlarm(this);"><span class="w50">保存</span></a>
    </div>
</body>
</html>
<script type="text/javascript">
    ...
    remoteConfig.drawMaskAlarm = function(param){
	...
	function setDrawEnabled(btn){
	    if(!getEnabled()){
	        cms.box.alert({title:'提示信息', html:'请先启用遮挡报警！'});
	        return false;
	    }
	    if(isDraw){
    ...
    //-----------以下是功能代码---------
	remoteConfig.getMaskAlarm = function(){
        var strDevCode = devInfo.devCode;
        var urlparam = 'action=getMaskAlarm&devCode=' + strDevCode + '&field=mask_alarm';

        module.appendDebugInfo(module.getDebugTime() + '[getMaskAlarm Request] param: ' + urlparam);
        remoteConfig.ajaxRequest({
            url: remoteConfig.getRemoteConfigUrl,
            data: urlparam,
            callBack: remoteConfig.getMaskAlarmCallBack
        });
    };
    
    remoteConfig.getMaskAlarmCallBack = function(data, param){
        if(!data.isJsonData()){
        ...
        remoteConfig.showMaskAlarm(jsondata, param);
    };
    
    remoteConfig.showMaskAlarm = function(jsondata, param){
        ...
        if('mask_alarm' == param){
            ...
            cms.util.setRadioChecked('rbAlarmSensitivity', info.sensitive);
        }
    };
    
    //remoteConfig.getMaskAlarm();
    
    remoteConfig.readMaskAlarm = function(btn){
        var strDevCode = devInfo.devCode;
        var channelNo = $('#ddlChannelId').val();
        
        var urlparam = 'action=getMaskAlarmEnabled&devCode=' + strDevCode + '&setting=' + channelNo;
        module.appendDebugInfo(module.getDebugTime() + '[readMaskAlarmEnabled Request] param: ' + urlparam);
        remoteConfig.ajaxRequest({
            data: urlparam,
            callBack: remoteConfig.readMaskAlarmCallBack,
            param: {
                btn: btn,
                field: 'mask_alarm_enabled'
            }
        });
        var urlparam = 'action=getMaskAlarm&devCode=' + strDevCode + '&setting=' + channelNo;        
        if(!remoteConfig.checkControlDisabled(btn)){
            return false;
        }
        module.appendDebugInfo(module.getDebugTime() + '[readMaskAlarm Request] param: ' + urlparam);
        remoteConfig.ajaxRequest({
            data: urlparam,
            callBack: remoteConfig.readMaskAlarmCallBack,
            param: {
    ...
    remoteConfig.readMaskAlarmCallBack = function(data, param){
        module.appendDebugInfo(module.getDebugTime() + '[readMaskAlarm Response] data: ' + data);
        ...
        remoteConfig.task.appendTaskList({
            id:id, action:'readMaskAlarm', title:'读取视频遮盖', result:'正在读取，请稍候...',
            callBack:'remoteConfig.showMaskAlarmResult',
            callBackParam: param.field
        });
    };
    
    remoteConfig.showMaskAlarmResult = function(strResult, bSuccess, strParam){
        if(!strResult.isXmlDom()){return false;}
        var strDevCode = devInfo.devCode;
        var urlparam = 'action=parseXmlProtocol&xml=' + strResult + '&field=' + strParam;

        module.appendDebugInfo(module.getDebugTime() + '[parseMaskAlarm Request] param: ' + urlparam);
        remoteConfig.ajaxRequest({
            url: remoteConfig.getRemoteConfigUrl,
            data: urlparam,
            callBack: remoteConfig.getMaskAlarmCallBack,
    ...
    if(remoteConfig.isAutoRead){
        remoteConfig.readMaskAlarm(cms.util.$('btnRead'));
    }
       
    remoteConfig.setMaskAlarm = function(btn){
        var strDevCode = devInfo.devCode;
        ...
        var arrNode = [
            ...
            ['sensitive', cms.util.getRadioCheckedValue('rbAlarmSensitivity', 1)]
        ];
        ...
        var urlparam = 'action=setMaskAlarmEnabled&devCode=' + strDevCode + '&setting=' + (remoteConfig.isEscapeXml ? escape(strXml) : strXml);
        
        module.appendDebugInfo(module.getDebugTime() + '[setMaskAlarmEnabled Request] param: ' + urlparam);
        remoteConfig.ajaxRequest({
            url: remoteConfig.setRemoteConfigUrl,
            data: urlparam,
            callBack: remoteConfig.setMaskAlarmCallBack,
            param: {
                btn: btn
            }
        });
        ...
        var urlparam = 'action=setMaskAlarm&devCode=' + strDevCode + '&setting=' + (remoteConfig.isEscapeXml ? escape(strXml) : strXml);
        if(!remoteConfig.checkControlDisabled(btn)){
            return false;
        }
        module.appendDebugInfo(module.getDebugTime() + '[setMaskAlarm Request] param: ' + urlparam);
        remoteConfig.ajaxRequest({
            url: remoteConfig.setRemoteConfigUrl,
            data: urlparam,
            callBack: remoteConfig.setMaskAlarmCallBack,
    ...
    remoteConfig.setMaskAlarmCallBack = function(data, param){
        module.appendDebugInfo(module.getDebugTime() + '[setMaskAlarm Response] data: ' + data);
        remoteConfig.setControlDisabledByTiming(param.btn, false);
        if(!data.isJsonData()){
            module.showJsonErrorData(data);
            return false;
        ...
        var id = jsondata.list[0];
        remoteConfig.task.appendTaskList({id:id, action:'setMaskAlarm', title:'设置视频遮盖', result:'正在设置，请稍候...', callBack:''});
    };
</script>