...
tree.buildTree = function(obj, config){
	...
	var urlparam = 'action=getTree&controlUnitId=0&treeType=' + config.treeType + '&keywords=' + escape(config.keys) 
	    + '&status2G=' + config.status2G + '&status3G=' + config.status3G + '&showGroup=' + 0 
	    + '&' + new Date().getTime();
...
tree.devContextMenu = function(e, obj, dev){
	...
	*/
	item[idx++] = {
		text:'实时视频预览', func:function(){
		    module.showPreview(dev.code, dev.name);
		}
	};
	item[idx++] = {
		text:'设备录像回放', func:function(){
		    module.showPlayBack(dev.code, dev.name);
		}
	};
	item[idx++] = {
		text:'GPS历史轨迹', func:function(){
		    module.showGpsTrack(dev.code, dev.name);
		}
	};
	var config = {
		width: 80,
...
tree.showDevDetail = function(id, code){
    var size = {w: 360, h:400};
    var strHtml = cms.util.path + '/modules/device/device.aspx?devId=' + id + '&devCode=' + code;// + cms.box.buildIframe(size.w, size.h);//tree.getDevInfo(id, code) + cms.box.buildIframe(size.w, size.h);
    var config = {
        id: 'previewdev',
        title: '设备详情' + cms.box.buildIframe(size.w, size.h),
        html: strHtml,
        noBottom: true,
        width: size.w,
        height: size.h,
    ...
    var pwDevInfo = cms.box.win(config);
};