...
var initialModulecIcon = function(){
    ...
    //生产管理 显示施工计划
    if(strMenuCode.equals('30104')){
        window.setTimeout(safety.workplan.showWorkPlan, 2000, 'btnWorkPlan');
    } else if(strMenuCode.equals('30105')){
        window.setTimeout(safety.dutyplan.showDutyPlan, 1000, '');
    }
...
var mapCallback = function(type, id, name, action){
    ...
    if('showWorkPlan' == action){
        ...
        var planId = arr[1];
        var workPlace = arr[2];
        deviceDetail.showWorkPlanDetail(planId);
        
        if('JCW' == planType){
            safety.workplan.showWorkPlace(planType, planId, workPlace);
        }
    } else if('showWorkPlace' == action){
        var arr = id.split('|');
        var planType = arr[0];
        var planId = arr[1];
        deviceDetail.showWorkPlanDetail(planId);
        ...
        switch(type){
            case 'pillar':
                ...
                //safety.device.showDeviceDetail(action, id, 'pillar', name);
                break;
            case '2':
                break;
            case '3':
                //showMarkerMenu(type, action, name);
                if(strMenuCode.equals('30105')){
                    deviceDetail.showQuestionLibraryDetail('BM', action, name);
                } else if(strMenuCode.equals('30106')){
                    //显示管界
                    boundary.showBoundary(action, true);                    
                } else if(strMenuCode.equals('30108')){
                    deviceDetail.showDepartmentPresonDetail('BM', action, name);
                }
                break;
            case '4':
                //showMarkerMenu(type, action, name);
                if(strMenuCode.equals('30104')){
                    //显示工班施工安排
                    safety.workplan.showDeptWorkPlan(action, name);
                } else if(strMenuCode.equals('30105')){
                    deviceDetail.showQuestionLibraryDetail('BM', action, name);
                } else if(strMenuCode.equals('30106')){
                    //显示管界
            ...
            case '8':
                //showMarkerMenu(type, action, name);
                if(strMenuCode.equals('30105')){
                    deviceDetail.showQuestionLibraryDetail('ST', action, name);
                } else if(strMenuCode.equals('30104') || strMenuCode.equals('30106')){
                    deviceDetail.showSubstationDetail(action, name);
                } else if(strMenuCode.equals('30108')){
                    deviceDetail.showDepartmentPresonDetail('ST', action, name);
                }
                break;
            case 'dutyplan':
                switch(action){
                    case 'dutyplan':
                        var arr = id.split('_');
                        var planId = arr[2];
                        deviceDetail.showDutyPlanDetail(planId);
                        break;
                    case 'workplan':
                        var arr = id.split('_');
                        var planId = arr[2];
...
var showMarkerMenu = function(type, code, name){
    var strType = '3' == type || '4' == type ? 'BM' : 'ST';
    var strHtml = '<div style="padding:10px;">'
        + '<a onclick="landmark.showIntroduce(\'{type:\\\'' + type + '\\\',code:\\\'' + code + '\\\',name:\\\'' + name + '\\\'}\');">简介</a>'
        + '<a onclick="deviceDetail.showQuestionLibraryDetail(\'' + strType + '\',\'' + code + '\',\'' + name + '\');">问题库</a>'
        + '<a onclick="deviceDetail.showDepartmentPresonDetail(\'' + strType + '\',\'' + code + '\',\'' + name + '\');">人员信息</a>'
        + '</div>';
    ...
    var config = {
        ...
        boxType: 'message',
        position: 8,
        y: 0//bodySize.height - boxSize.height + 25
    };
    cms.box.win(config);
};