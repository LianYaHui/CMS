...
sms.curMenu = '';
sms.curPageUrl = '';
sms.funcName = 'sms.treeAction';
sms.cmsPath = cms.util.path + '/modules/sms/sub';
sms.arrTreeData = {
	module:[
		{id: 0, pid: -1, name: ''},
		{id: 1, pid: 0, name: '短信管理', func: null},
		{id: 2, pid: 0, name: '电话录音管理', func: null}
	],
	menu:[
	    {id:101, pid: 1, name: '短信发送', callBack:{func: sms.funcName, action: 'send', url: sms.cmsPath + '/send.aspx'}},
		{id:102, pid: 1, name: '短信发送记录', callBack:{func: sms.funcName, action: 'send_log', url: sms.cmsPath + '/sendLog.aspx'}},
		{id:103, pid: 1, name: '短信接收记录', callBack:{func: sms.funcName, action: 'receive', url: sms.cmsPath + '/receive.aspx'}},
	    {id:201, pid: 2, name: '电话录音管理', callBack:{func: sms.funcName, action: 'sound_record', url: sms.cmsPath + '/soundRecord.aspx'}}	
	]
};

$(window).load(function(){
    
    cms.frame.setFrameSize(sms.frame.bodyLeftWidth, 0);
    
	sms.frame.initialForm();
	sms.frame.setBodySize();
	sms.frame.setFrameByShortcutKey();
	sms.loadTreeMenu();
    
	sms.initialForm();
	sms.frame.setBodySize();
	
	window.setTimeout(sms.loadDefaultPage, 50);
	
    cms.frame.setFrameByShortcutKey({resizeFunc: sms.frame.setBodySize, keyFunc: null, shiftKeyFunc: null});
});
...
sms.loadTreeMenu = function(){
	sms.tree.buildMenuTree(sms.arrTreeData, [1,2], sms.arrTreeData.module.length);
};

sms.loadDefaultPage = function(){    
	sms.treeAction({type: 'menu', action:'send', name: '短信发送', url: sms.cmsPath + '/send.aspx', close: true});	//加载默认页面
};

sms.treeAction = function(param){
	if(sms.curSelect == param.action){
		return false;	//不重复加载
	}
	var strUrl = '';
	if(param.type == 'menu'){
		//改变框架中标题栏的名称
		sms.frame.setFormTitle(param.name);
		sms.frame.setFormPrompt('');
		strUrl = param.url;
	}
	if(strUrl == ''){
	    strUrl = 'about:blank';
	...
	sms.curSelect = param.action;	
};

sms.setFormBodySize = function(callBack){
    var formSize = sms.frame.getFormSize();
    var conHeight = formSize.height - sms.frame.paddingTop - sms.frame.bodyBottomHeight;
    $('#divFormContent').height(conHeight);
    
    if(typeof callBack == 'function'){
        sms.resizeCallBackFunc = callBack;
    } else if(typeof callBack == 'string' && typeof eval('(' + callBack + ')') == 'function'){
        sms.resizeCallBackFunc = eval('(' + callBack + ')');
    }
    if(sms.resizeCallBackFunc != null){
        sms.resizeCallBackFunc(formSize, conHeight);
    }
};

sms.setFormTableStyle = function(){
