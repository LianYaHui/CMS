...
cms.preset.unloadControl = function(){
    cms.util.clearDataRow(cms.util.$('tbPreset'), 0);
    $('#presetPanel').html('');
    $('#presetToolbar').html('');
    cms.preset.pnsobj = null;
};
...
cms.preset.initialPresetList = function(obj, count){
    ...
    for(var i=0; i<count; i++){
        ...
        var strPresetObj = '<span id="lblPresetName_' + pn + '">' + strPresetName + '</span>';
        var strFlag = '<span id="lblIsPreset_' + pn + '" style="display:none;" />0</span>'
            + '<span id="lblPresetDeleted_' + pn + '" class="flag-right" style="width:25px;height:22px;display:none;"></span>';
            
        if(0 == i){
            rowData[cellid++] = {html: pn, style:[['width','35px'],['textAlign', 'center']]}; //序号
            rowData[cellid++] = {html: strPresetObj, title:strTitle, style:[['width','139px']]}; //名称
            rowData[cellid++] = {html: strFlag, style:[['width','25px']]}; //标识
        } else {
            rowData[cellid++] = {html: pn, style:[['textAlign', 'center']]}; //序号
            rowData[cellid++] = {html: strPresetObj, title:strTitle, style:[]}; //名称
            rowData[cellid++] = {html: strFlag,  style:[]}; //标识        
        }
...
cms.preset.resetPresetInfo = function(){
    ...
    for(var i=0; i<cms.preset.maxCount; i++){
        pn = i + 1;
        if(cms.util.$('lblPresetName_' + pn) == null || cms.util.$('lblPresetDeleted_' + pn) == null){
            break;
        }
        cms.util.$('lblPresetName_' + pn).innerHTML = '预置点' + pn;
        cms.util.$('lblIsPreset_' + pn).innerHTML = '0';
...
cms.preset.setPreset = function(pn){
    ...
    var strPresetName = cms.util.$('lblPresetName_' + pn).innerHTML.trim();
    var iAction = 1;
...
cms.preset.fillPresetInfo = function(jsondata, presetNo){
    ...
    if(c > 0){
        for(var i=0; i<c; i++){
            cms.util.$('lblPresetName_' + jsondata.list[i].no).innerHTML = jsondata.list[i].name;
            var isPreset = jsondata.list[i].deleted == 0;
            cms.util.$('lblIsPreset_' + jsondata.list[i].no).innerHTML = isPreset ? '1' : '0';
            cms.util.$('lblPresetDeleted_' + jsondata.list[i].no).style.display = isPreset ? 'block' : 'none';
        }
    }
};
...
cms.preset.editPresetName = function(obj){
    ...
    var strName = $('#lblPresetName_' + cms.preset.presetNo).html();
    var isPreset = $('#lblIsPreset_' + cms.preset.presetNo).html();
...
cms.preset.deletePesetInfo = function(cameraId, presetNo){
    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR){
            ...
            var jsondata = strData.toJson();//eval('(' + data + ')');
            if(jsondata.result == 1){
...
cms.preset.changePresetName = function(presetNo, strPresetName){
    var obj = cms.util.$('lblPresetName_' + presetNo);
    if(obj != null){
        obj.innerHTML = strPresetName;
    }
};