...
var paddingWidth = 5;
var borderWidth = 2;
var leftWidth = leftWidthConfig = 222;
...
//是否处理OCX 设备上下线消息事件
cms.preview.isDisposeOcxDevOnOffEvent = false;
...
//当前窗口数量
cms.preview.curWinCount = 1;
//播放窗口与设备记录
cms.preview.arrWinDev = [];

cms.preview.curDevCode = '';
cms.preview.channelNo = 0;
...
$(window).load(function(){
    ...
    if($('#txtChannelCount').val().trim().equals('1') && $('#txtDevCode').val().trim().indexOf(',') < 0){
        treeHeight = 100;
    }
    setBodySize();
...
var initialForm = function(){
    ...
    if(!cms.util.isMSIE){
        ...
        var previewOcx = cms.util.$('previewOcx');
        if(previewOcx != null){
...
var initialOcx = function(){
    ...
    if(autoPlay){
        window.setTimeout(cms.preview.autoPreview, 6000);
    }
};
...
cms.preview.autoPreview = function(){
    var devId = $('#txtDevId').val();
    cms.preview.channelNo = parseInt($('#txtChannelNo').val());
    if(cms.preview.channelNo > 0){
        var param = {devCode: $('#txtDevCode').val(), channelNo: cms.preview.channelNo };
        //设置树菜单选中节点
        //tree.selectNode(cms.preview.channelNo + 1);
        
        cms.preview.devVedioPreview(devId, param);
    }
};
...
var setPresetPanelSize = function(){
    var bh = [cms.preview.ptzPanelH];
    var presetH = getBoxHeight() - (cms.preview.ptzPanelShow ? bh[0] : 0);
    $('#presetBox').height(presetH);
    $('#presetPanel').height(presetH - cms.preview.presetBarH - 26 - 2);
    $('#presetToolbar').height(cms.preview.presetBarH - 1);
};

var getBoxHeight = function(){
    var frameSize = cms.frame.getFrameSize();
    return frameSize.height - borderWidth - paddingTop*2 - (cms.util.isMSIE ? 0 : 1) - treeHeight - 25;
};
...
var setBoxSize = function(){
    ...
    //$('#leftMenu').height(boxSize.height - 25 - cms.preview.ptzPanelH);
    $('#treebox').width(leftWidth - 2);
    //$('#treebox').height(boxSize.height - 25*2 - cms.preview.ptzPanelH);
    $('#treebox').height(treeHeight - 25);
    
    $('#ptzSpeedBox').width(leftWidth - 10);
    ...
    $('.previewOcx').width(boxSize.width);
    $('.previewOcx').height(boxSize.height - 25);
};
...
cms.preview.play = function(winNum, devCode, channelNo, streamType, connectType, devLineId, winParam){
    ...
    */
    module.appendOcxDebugInfo(module.getOcxDebugTime() + '[Play Request] szDevId: ' + devCode + ', iChannel: ' + channelNo 
        + ', iSubChannel: ' + streamType + ', iTransMode: ' + connectType + ', iStreamCtrl: ' + 0 + ', iDevLine: ' + devLineId);
...
cms.preview.setAudio = function(btn){
    ...
    switch(btn.rel){
        case 'close':
            ...
            module.appendOcxDebugInfo(module.getOcxDebugTime() + '[WMPSoundVolume Response] return: ' + cms.ocx.showErrorCode(result));
            ...
            module.appendOcxDebugInfo(module.getOcxDebugTime() + '[WMPPlaySound Response] return: ' + cms.ocx.showErrorCode(result));
        ...
        case 'open':
            ...
            module.appendOcxDebugInfo(module.getOcxDebugTime() + '[WMPStopSound Response] return: ' + cms.ocx.showErrorCode(result));
            cms.preview.curPlayAudioWinNum = -1;
            /*
            if(0 == result){
                cms.preview.arrWinDev[cms.preview.curWinNum].playAudio = btn.rel;
