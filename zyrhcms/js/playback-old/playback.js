...
$(window).load(function(){
    ...
    //加载树型菜单
    tree.showTreeMenu(false, treeType, 'treeAction', false, true, false, false, true, '3,4');
    //定时刷新树型菜单设备状态
...
var setRightPanelSize = function(){
    ...
    $('.listbox').width(rightWidth);
    if(cms.util.isIE6){
        $('.listbox .listheader').width(rightWidth - 2);
        $('.listbox .list').width(rightWidth - 2);
    }
    $('.listbox').height(boxSize.height - cms.playback.searchFormPanelH - 26 - 25);
    $('.listbox .listheader').height(23);
    $('.listbox .list').height(boxSize.height - cms.playback.searchFormPanelH - 25 - 23 - 25);
...
var searchFile = function(btn){
    ...
    var fileType = parseInt($('#ddlFileType').val(), 10);
...
cms.playback.buildVideoControl = function(){
    var strHtml = ''
        ...
        */
        ...
        /*
        + '<a name="winset" class="winset cur1" onclick="cms.playback.setWindowRank(1, this);" title="一画面">1画面</a>'
        + '<a name="winset" class="winset win4" onclick="cms.playback.setWindowRank(4, this);" title="四画面">4画面</a>'
        */
...
cms.playback.showCentralRecords = function(strJson){
    ...
    try{
		...
		if(count > 0){
		    cms.playback.showCentralRecordListContent(arrFile);
		    cms.playback.showPrompt('');
		    cms.playback.showFileStat('设备“' + devName + '” 共搜索到 ' + count + ' 个录像文件');
		} else {
		    //没有搜索到录像文件
...
cms.playback.buildStoreListContent = function(row, dr, rid){
	...
	var strPlayBack = '<a class="playback" title="单击回放录像" id="btnPlayBack_' + (rid - 1) + '" '
	    + 'onclick="startStorePlayBack(this, \'' + cms.playback.curDevCode + '\',' + cms.playback.channelNo + ',\'' + dr.begin_time + '\',\'' + dr.end_time + '\',' + dr.type + ',' + dr.size + ');"'
	    + '></a>';
...
var startPlayBack = function(obj, strFileName, fileSize){
    ...
    //szDevId, iChannel, szBeginTime, szEndTime, iRecFlag, nTotalSize
};

var startStorePlayBack = function(obj, devCode, channelNo, startTime, endTime, fileType, nTotalSize, idx){
    cms.playback.recordFileIndex = idx;
    
    var tr = obj.parentNode.parentNode;
    //设置回放按钮所在行为选中行
    if(cms.playback.setRowSelected(tr)){
        window.setTimeout(cms.playback.startStorePlayBack, 100, devCode, channelNo, startTime, endTime, fileType, nTotalSize);
        
        //szDeviceId, iChannel, szFileName, nOffset, nReadSize, nTotalSize, iSaveToCenter, iDevLine
    }
};
...
cms.playback.startStorePlayBack = function(devCode, channelNo, startTime, endTime, fileType, dataSize, dragPlay, idx){
    if(cms.playback.isPlayBack){
        cms.playback.stopPlayBack(dragPlay);
    }
    module.appendOcxDebugInfo(module.getOcxDebugTime() + '[PlayCentralRecord Request] szDevId: ' + devCode + ', iChannel: ' + channelNo + ', szBeginTime: ' 
        + startTime + ', szEndTime: ' + endTime + ', iRecFlag: ' + fileType + ', nTotalSize: ' + dataSize);
debugger;
    var result = cms.ocx.playCentralRecord(cms.playback.previewOcx, devCode, channelNo, startTime, endTime, fileType, dataSize);
...
cms.playback.playEvent = function(szDevId, iChannel, nRecFlag, nDataSize, szFrameTime){alert(szFrameTime);
    ...
    if(cms.playback.continuous && parseInt(nRecFlag, 10) == 2){
        if(cms.playback.recordFileIndex < cms.playback.arrRecordFile.length - 1){
            ...
            startStorePlayBack(cms.util.$('btnPlayBack_' + idx), cms.playback.storeServerIp, cms.playback.storeServerPort, cms.playback.curDevCode, cms.playback.channelNo,
